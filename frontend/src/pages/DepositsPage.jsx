        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            ...
            <table className="hidden md:table w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 sm:px-6 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Concesionario</th>
                  <th className="px-4 py-3 sm:px-6 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Municipio</th>
                  <th className="px-4 py-3 sm:px-6 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Dirección</th>
            ...
            <div className="md:hidden flex flex-col divide-y divide-gray-100">
              {filteredDepositos.map((deposito) => (
                <div key={deposito.id} className="p-4 hover:bg-gray-50 transition-colors">
                  {/* Top row: Name, Number, Status */}
                  <div className="flex justify-between items-start mb-3">
                    <div className="pr-2">
                       <span className="text-sm font-medium text-(--color-primary) mb-0.5 block">#{deposito.numero}</span>
                       <div className="text-sm font-semibold text-gray-900 leading-tight">{deposito.nombrePropietario}</div>
                    </div>
                    <span className={`shrink-0 inline-flex px-2 py-0.5 text-[10px] font-medium rounded-full ${getStatusColor(deposito.activo ? 'ACTIVO' : 'INACTIVO')}`}>
                      {deposito.activo ? 'ACTIVO' : 'INACTIVO'}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs mb-3">
                    <div>
                      <p className="font-semibold text-gray-500 mb-0.5">Municipio</p>
                      <p className="text-gray-900 truncate" title={deposito.municipio}>{deposito.municipio}</p>
                    </div>
                    <div>
                      <p className="font-semibold text-gray-500 mb-0.5">Dirección</p>
                      <p className="text-gray-900 truncate" title={deposito.direccion}>{deposito.direccion}</p>
                    </div>
                  </div>
